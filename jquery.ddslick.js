    ...
    methods.init = function (options) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                ...
                $.each(options.data, function (index, item) {
                    ddOptions.append('<li>' +
                        '<a ' + (item.href === undefined ? '' : 'href="' + item.href + '" target="' + item.target + '"') + ' class="dd-option">' +
                            ...
                            (item.text ? ' <label tabIndex=0 class="dd-option-text" style="cursor:pointer;">' + item.text + '</label>' : '') +
                ...
                obj.find('.dd-select').bind('keypress', function (e) {
                ...
                obj.find('.dd-select').click( function () {
                    var target = document.activeElement;
                    if (target && !target.id){
                      var randLetter = String.fromCharCode(65 + Math.floor(Math.random() * 26));
                      var uniqid = randLetter + Date.now();
                ...
                //Selecting an option
                obj.find('.dd-option').click( function () {
        ...
        if (ddSelected.text().length == 0 || !settings.resetmenu) {
            if (settings.showSelectedHTML) {
                ddSelected.html(
                        ...
                        (selectedData.text ? '<label tabIndex=0 class="dd-selected-text" style="cursor:pointer;">' + selectedData.text + '</label>' : '') +
